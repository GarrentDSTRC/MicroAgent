...
def load_model_for_evaluation(model_path, adapter_path=None, device="cuda", base_model_path=None):
    """
    ...
    if is_lora or adapter_path:
        ...
        # 优先使用传入的base_model_path，否则从adapter_config.json中获取
        if base_model_path:
    ...
    if is_finetuned:
        ...
        for model_path in model_paths:
            if os.path.exists(model_path):
                ...
                try:
                    # 首先尝试直接加载
                    model, processor = load_model_for_evaluation(model_path, device=str(device), base_model_path=config['base_model_path'])
                    loaded_model = True
                    ...
                    if os.path.isdir(model_path):
                        ...
                        for subdir in subdirs_sorted:
                            ...
                            try:
                                model, processor = load_model_for_evaluation(subdir_path, device=str(device), base_model_path=config['base_model_path'])
                                loaded_model = True
                ...
                for img_path in image_paths:
                    ...
                    if img_path.startswith('data:image'):
                        ...
                        possible_paths = [
                            ...
                            os.path.join('D:/MicroAgent/Stage2', img_path.lstrip('./')),  # 绝对路径
                            r"D:\MicroAgent\Stage2\data\vlm_finetune_dataset_fixed\images" + img_path.replace('./data/vlm_finetune_dataset_fixed/images', ''),
                        ]
